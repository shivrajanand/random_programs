...
def transparent_layout(img, buttonList):
    ...
    for button in buttonlist:
        x, y = button.pos
        cvzone.cornerRect(
            imgNew, (button.pos[0], button.pos[1], button.size[0], button.size[0]), 20, rt=0)
        cv2.rectangle(imgNew, button.pos(
            x + button.size[0], y + button.size[1]), (255, 144, 30), cv2.FILLED)
        ...
        print(mask.shape)
        out[mask] = cv2.addWeighted(img, alpaha, imgNew, 1-alpaha, [mask])
        return out
